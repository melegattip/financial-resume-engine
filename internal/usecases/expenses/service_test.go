...
func (m *MockExpenseRepository) ListByDueDate(userID string) ([]*domain.Expense, error) {
	args := m.Called(userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockExpenseRepository) MarkAsPaid(userID string, expenseID string) error {
	args := m.Called(userID, expenseID)
	return args.Error(0)
}
...
func TestGetExpense(t *testing.T) {
	...
	assert.Equal(t, expectedExpense.Category, result.Category)
...
func TestUpdateExpense(t *testing.T) {
	...
	request := &UpdateExpenseRequest{
		...
		Paid:        true,
		DueDate:     time.Now().AddDate(0, 1, 0).Format("2006-01-02T15:04:05Z07:00"),
	}
...
func TestMarkAsPaid(t *testing.T) {
	...
	mockRepo.On("MarkAsPaid", "user1", "1").Return(nil)
	mockRepo.On("Get", "user1", "1").Return(expense, nil)

	result, err := service.MarkAsPaid(context.Background(), "user1", "1")
	assert.NoError(t, err)
